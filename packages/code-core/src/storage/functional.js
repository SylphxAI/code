 ...
 */
export const createFilesystemOps = (config) => {
    const resolvePath = (key) => path.join(config.basePath, key);
    const put = async (key, content, options) => {
        try {
 ...
 */
export const createS3Ops = (config) => {
 ...
 */
export const createStorageOps = (config) => {
    switch (config.type) {
        case "filesystem":
 ...
 */
export const getStorageConfigFromEnv = (env = readEnv) => {
 ...
 */
export const withLogging = (ops, logger) => ({
 ...
 */
export const withRetry = (ops, maxRetries = 3, delayMs = 1000) => {
 ...
 */
export const withCache = (ops, cache, ttlMs = 60000) => ({
    put: async (key, content, options) => {
        const result = await ops.put(key, content, options);
        if (result.success) {
