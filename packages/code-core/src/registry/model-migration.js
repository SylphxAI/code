 ...
 */
export function migrateToModelId(provider, model) {
 ...
 */
export function getDefaultModelIdForProvider(provider) {
    const defaults = {
        openai: "gpt-4o",
 ...
 */
export function getProviderIdFromModelId(modelId) {
 ...
 */
export function migrateSessionModel(session) {
    // Already has modelId
    ...
    if (session.provider && session.model) {
        ...
        if (defaultModelId) {
            console.warn(`[Model Migration] Using default model for provider=${session.provider}: ${defaultModelId}`);
            return {
                ...session,
    ...
    console.error(`[Model Migration] Failed to migrate session model, using fallback: ${firstModelId}`);
    return {
        ...session,
        modelId: firstModelId,
    };
}
//# sourceMappingURL=model-migration.js.map