...
export async function retry(operation, options = {}) {
    ...
    for (let attempt = 0; attempt <= opts.maxRetries; attempt++) {
        try {
            ...
            const delay = Math.min(opts.initialDelayMs * opts.backoffMultiplier ** attempt, opts.maxDelayMs);
 ...
 */
export function retryAllErrors() {
    return true;
}
/**
 * Convenience function: Retry database operations with SQLITE_BUSY handling
 */
export async function retryDatabase(operation, maxRetries = RETRY.DATABASE_MAX_RETRIES) {
 ...
 */
export async function retryNetwork(operation, maxRetries = RETRY.NETWORK_MAX_RETRIES) {
    return retry(operation, {
        maxRetries,
        initialDelayMs: RETRY.INITIAL_DELAY_MS,
