 ...
 */
export function getServerURL(host = NETWORK.DEFAULT_HOST, port = NETWORK.DEFAULT_PORT) {
    return `http://${host}:${port}`;
}
/**
 * Helper: Get web UI URL
 */
export function getWebURL(host = NETWORK.DEFAULT_HOST, port = NETWORK.DEFAULT_WEB_PORT) {
    return `http://${host}:${port}`;
}
/**
 * Helper: Get database path
 */
export function getDatabasePath(cwd = process.cwd()) {
    return `${cwd}/${DATABASE.DEFAULT_PATH}`;
}
/**
 * Helper: Get config directory path
 */
export function getConfigDir(homeDir = process.env.HOME || process.cwd()) {
