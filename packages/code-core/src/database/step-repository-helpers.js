...
import { messageSteps, stepParts, stepUsage } from "./schema.js";
/**
 ...
 */
export async function createMessageStep(db, messageId, stepIndex, _metadata, _todoSnapshot, systemMessages, provider, model, stepId) {
    const actualStepId = stepId || `${messageId}-step-${stepIndex}`;
    const now = Date.now();
    await retryDatabase(() => db.transaction(async (tx) => {
        // Insert step
        const newStep = {
            id: actualStepId,
            messageId,
 ...
 */
export async function updateStepParts(db, stepId, parts) {
    await retryDatabase(() => db.transaction(async (tx) => {
 ...
 */
export async function completeMessageStep(db, stepId, options) {
 ...
 */
export async function loadMessageSteps(db, messageId) {
        ...
        if (step.systemMessages) {
            try {
                ...
                if (parsedSys.success) {
                    ...
                    logger.error("Failed to validate systemMessages", parsedSys.error);
                }
            }
            catch (error) {
                logger.error("Failed to parse systemMessages", error);
            }
        }
...
//# sourceMappingURL=step-repository-helpers.js.map