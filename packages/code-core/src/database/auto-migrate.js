        ...
        try {
            ...
            for (const message of session.messages) {
                ...
                if (message.attachments &&
                    ...
                    const validAttachments = message.attachments.filter((a) => a !== null &&
                        typeof a === "object" &&
                        "path" in a &&
                        "relativePath" in a &&
            ...
            logger.error(`Error migrating ${sessionId}`, error);
            errorCount++;
            // Keep JSON file on error for debugging
        }
    }
    // Create migration flag
    await fsReadFile(MIGRATION_FLAG, "utf8").catch(() => require("node:fs").writeFileSync(MIGRATION_FLAG, new Date().toISOString()));
