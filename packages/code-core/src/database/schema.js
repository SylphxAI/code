...
export const sessions = sqliteTable("sessions", {
    ...
    enabledRuleIds: text("enabled_rule_ids", { mode: "json" })
        .notNull()
        .default("[]")
        .$type(), // Enabled rules for this session
    // NEW: Normalized tool and MCP server IDs
    enabledToolIds: text("enabled_tool_ids", { mode: "json" }).$type(), // References Tool.id[]
    enabledMcpServerIds: text("enabled_mcp_server_ids", { mode: "json" }).$type(), // References MCPServer.id[]
...
export const todos = sqliteTable("todos", {
    ...
    relatedFiles: text("related_files", { mode: "json" }).$type(), // Related file paths
    metadata: text("metadata", { mode: "json" }).$type(), // Additional metadata
}, (table) => ({
...
export const events = sqliteTable("events", {
    ...
    payload: text("payload", { mode: "json" }).$type().notNull(), // Event data as JSON
    createdAt: integer("created_at").notNull(), // When saved to DB
}, (table) => ({
    // Composite index for cursor-based queries (channel + cursor)
    channelCursorIdx: index("idx_events_channel_cursor").on(table.channel, table.timestamp, table.sequence),
