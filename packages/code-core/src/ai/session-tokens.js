...
const baseContextCache = new LRUCache(100);
// Register with cache manager
cacheManager.register("base-context", baseContextCache, "Base context tokens (system prompt + tools) with SHA256 content-based caching");
/**
 * Generate content hash for base context caching
 * Hash includes all content that affects token count
 */
function generateBaseContextHash(agentId, allAgents, enabledRules, tools) {
    // 1. Agent content
 ...
 */
export async function calculateBaseContextTokens(modelName, agentId, enabledRuleIds, cwd, options) {
 ...
 */
export function clearBaseContextCache() {
