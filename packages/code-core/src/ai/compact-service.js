 ...
 */
export async function compactSession(sessionRepository, sessionId, providerConfig, onProgress) {
    try {
        ...
        const newSession = await sessionRepository.createSession(session.provider, session.model, session.agentId || "coder", session.enabledRuleIds || []);
 ...
 */
export function shouldCompactSession(session, thresholds = {}) {
