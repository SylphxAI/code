...
export class ClaudeCodeLanguageModel {
    ...
    async doGenerate(options) {
        try {
            logger.debug("Starting doGenerate", { model: this.modelId });
            ...
            const { prompt: promptString, shouldForceNewSession, messageFingerprints, } = convertMessagesToString(options, isResuming);
    ...
    doStream(options) {
        try {
            logger.debug("Starting doStream", { model: this.modelId });
            // Convert tools and build query options
            const tools = convertTools(options.tools || []);
