 ...
 */
export function detectMessageInconsistency(messages, lastProcessedCount, lastMessageFingerprints) {
 ...
 */
export function convertMessagesToString(options, isResuming) {
    const promptParts = [];
    const messages = options.prompt;
    // Extract provider options
    const providerOptions = options.providerOptions?.["claude-code"];
    const lastProcessedCount = providerOptions &&
        "lastProcessedMessageCount" in providerOptions &&
        typeof providerOptions.lastProcessedMessageCount === "number"
        ? providerOptions.lastProcessedMessageCount
        : undefined;
    const lastMessageFingerprints = providerOptions &&
        "messageFingerprints" in providerOptions &&
    ...
    if (isResuming && lastProcessedCount !== undefined) {
        const inconsistent = detectMessageInconsistency(messages, lastProcessedCount, lastMessageFingerprints);
        if (inconsistent) {
