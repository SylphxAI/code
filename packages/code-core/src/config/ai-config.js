 ...
 */
export const getProviderApiKey = async (config, providerId) => {
    const providerConfig = config.providers?.[providerId];
    if (!providerConfig) {
        return undefined;
    }
    ...
    if (providerConfig.credentialId) {
        try {
            ...
            const credential = getCredential(providerConfig.credentialId);
 ...
 */
export const getProviderConfigWithApiKey = async (config, providerId) => {
    const providerConfig = config.providers?.[providerId];
    if (!providerConfig) {
        return undefined;
    }
...
export const loadAIConfig = async (cwd = process.cwd()) => {
    return tryCatchAsync(async () => {
        ...
        return merged;
    }, (error) => new Error(`Failed to load AI config: ${error instanceof Error ? error.message : String(error)}`));
};
/**
...
export const saveAIConfig = async (config, cwd = process.cwd()) => {
    ...
    }, (error) => new Error(`Failed to save AI config: ${error instanceof Error ? error.message : String(error)}`));
};
/**
...
export const saveAIConfigTo = async (config, location, cwd = process.cwd()) => {
    ...
    return tryCatchAsync(async () => {
        ...
        // Write config
        await fs.writeFile(configPath, `${JSON.stringify(validated, null, 2)}\n`, "utf8");
    }, (error) => new Error(`Failed to save AI config to ${location}: ${error instanceof Error ? error.message : String(error)}`));
};
/**
...
export const migrateLegacyConfig = async (cwd = process.cwd()) => {
    ...
    }, (error) => new Error(`Failed to migrate legacy config: ${error instanceof Error ? error.message : String(error)}`));
};
