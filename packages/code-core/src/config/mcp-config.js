...
import { z } from "zod";
import { success, tryCatchAsync } from "../ai/result.js";
 ...
 */
export const getMCPConfigPath = (cwd = process.cwd()) => {
    return path.join(cwd, MCP_CONFIG_FILE);
};
/**
 * Load MCP servers configuration
 */
export const loadMCPConfig = async (cwd = process.cwd()) => {
    ...
    }, (error) => new Error(`Failed to load MCP config: ${error instanceof Error ? error.message : String(error)}`));
};
/**
 * Save MCP servers configuration
 */
export const saveMCPConfig = async (config, cwd = process.cwd()) => {
    const configPath = getMCPConfigPath(cwd);
    return tryCatchAsync(async () => {
 ...
 */
export const mcpConfigExists = async (cwd = process.cwd()) => {
    const configPath = getMCPConfigPath(cwd);
    try {
 ...
 */
export const addMCPServer = async (id, serverConfig, cwd = process.cwd()) => {
 ...
 */
export const updateMCPServer = async (id, updates, cwd = process.cwd()) => {
    return tryCatchAsync(async () => {
        // Load current config
        const configResult = await loadMCPConfig(cwd);
        if (!configResult.success) {
    ...
    }, (error) => new Error(`Failed to update MCP server: ${error instanceof Error ? error.message : String(error)}`));
};
/**
 * Remove MCP server from configuration
 */
export const removeMCPServer = async (id, cwd = process.cwd()) => {
    return tryCatchAsync(async () => {
        // Load current config
        const configResult = await loadMCPConfig(cwd);
        if (!configResult.success) {
    ...
    }, (error) => new Error(`Failed to remove MCP server: ${error instanceof Error ? error.message : String(error)}`));
};
/**
 * Get MCP server by ID
 */
export const getMCPServer = async (id, cwd = process.cwd()) => {
    return tryCatchAsync(async () => {
        const configResult = await loadMCPConfig(cwd);
        if (!configResult.success) {
            throw configResult.error;
    ...
    }, (error) => new Error(`Failed to get MCP server: ${error instanceof Error ? error.message : String(error)}`));
};
/**
 * List all MCP servers
 */
export const listMCPServers = async (cwd = process.cwd()) => {
    return tryCatchAsync(async () => {
        const configResult = await loadMCPConfig(cwd);
        if (!configResult.success) {
            throw configResult.error;
    ...
    }, (error) => new Error(`Failed to list MCP servers: ${error instanceof Error ? error.message : String(error)}`));
};
/**
 * List enabled MCP servers
 */
export const listEnabledMCPServers = async (cwd = process.cwd()) => {
    return tryCatchAsync(async () => {
    ...
    }, (error) => new Error(`Failed to list enabled MCP servers: ${error instanceof Error ? error.message : String(error)}`));
};
/**
 * Enable MCP server
 */
export const enableMCPServer = async (serverId, cwd = process.cwd()) => {
    return updateMCPServer(serverId, { enabled: true }, cwd);
};
/**
 * Disable MCP server
 */
export const disableMCPServer = async (serverId, cwd = process.cwd()) => {
    return updateMCPServer(serverId, { enabled: false }, cwd);
};
/**
 * Validate MCP server configuration
 */
export const validateMCPServerConfig = (server) => {
    try {
        ...
        return {
            ok: false,
            error: new Error(`Invalid MCP server config: ${error instanceof Error ? error.message : String(error)}`),
        };
    }
};
