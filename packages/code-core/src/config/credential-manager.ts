 ...
 */
export async function loadCredentials(cwd: string = process.cwd()): Promise<Result<void, Error>> {
 ...
 */
export async function saveCredentials(cwd: string = process.cwd()): Promise<Result<void, Error>> {
	return tryCatchAsync(
		async () => {
			const paths = getCredentialPaths(cwd);
