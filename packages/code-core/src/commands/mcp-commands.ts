...
export const mcpAddCommand: Command = {
	...
	handler: async (context: CommandContext) => {
		try {
			...
			let transportConfig: MCPTransport;
			...
			if (transport === "http") {
				...
				transportConfig = {
					...
					args: args ? (args as string).split(",").map((s) => s.trim()) : undefined,
				};
			} else {
...
export const mcpRemoveCommand: Command = {
	...
	handler: async (context: CommandContext) => {
		try {
			...
			if (mcpManager.isConnected(id as string)) {
				const disconnectResult = await mcpManager.disconnect(id as string);
			...
			// Remove server
			const result = await removeMCPServer(id as string);
			if (!result.ok) {
				return err(result.error);
			}
...
export const mcpEnableCommand: Command = {
	...
	handler: async (context: CommandContext) => {
		try {
			...
			const result = await enableMCPServer(id as string);
			if (!result.ok) {
				return err(result.error);
			}
...
export const mcpDisableCommand: Command = {
	...
	handler: async (context: CommandContext) => {
		try {
			...
			const result = await disableMCPServer(id as string);
			if (!result.ok) {
				return err(result.error);
			}
 ...
 */
export const mcpConnectCommand: Command = {
	name: "mcp:connect",
	...
	handler: async (context: CommandContext) => {
		try {
			...
			// Load tools
			await reloadMCPServerTools(id as string);

			const state = await mcpManager.getServerState(id as string);
 ...
 */
export const mcpDisconnectCommand: Command = {
	name: "mcp:disconnect",
	...
	handler: async (context: CommandContext) => {
		try {
			...
			const mcpManager = getMCPManager();
			const result = await mcpManager.disconnect(id as string);
			if (!result.ok) {
				return err(result.error);
			}
 ...
 */
export const mcpReconnectCommand: Command = {
	name: "mcp:reconnect",
	...
	handler: async (context: CommandContext) => {
		try {
			...
			const mcpManager = getMCPManager();
			const result = await mcpManager.reconnect(id as string);
			if (!result.ok) {
				return err(result.error);
			}
