...
export function useAskToolHandler({
	...
	const handleAskQuestion = useCallback(
		(
			...
			addDebugLog(
				`[ask tool] Received question (${questions.length} question${questions.length > 1 ? "s" : ""})`,
			);
			...
			inputResolver.current = async (answers: string | Record<string, string | string[]>) => {
				...
				try {
					...
					addDebugLog(
						`[ask tool] Error sending answer: ${error instanceof Error ? error.message : String(error)}`,
					);
					throw error;
				}
			};
