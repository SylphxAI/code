    ...
    for (const tagMatch of matches) {
        ...
        if (!cursorRendered &&
            ...
            if (isValidTag || tagMatch.type === "image") {
                ...
                parts.push(_jsxs(Text, { backgroundColor: bgColor, color: fgColor, children: [leftPart, showCursor && _jsx(Text, { inverse: true, children: rightPart.length > 0 ? rightPart[0] : " " }), rightPart.slice(1)] }, `tag-${partIndex}`));
            }
            else {
            ...
            if (isValidTag || tagMatch.type === "image") {
                // Image tags are always highlighted (no validation needed)
                parts.push(_jsx(Text, { backgroundColor: bgColor, color: fgColor, children: tagMatch.matchText }, `tag-${partIndex}`));
            }
            else {
                // Invalid file tag - render as normal text
                parts.push(_jsx(Text, { children: tagMatch.matchText }, `tag-${partIndex}`));
            }
        }
        partIndex++;
        lastIndex = matchEnd;
