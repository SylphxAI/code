...
import { checkAllTriggers, completeMessageStep, createMessageStep, updateStepParts, } from "@sylphx/code-core";
 ...
 */
export async function prepareStep(stepNumber, assistantMessageId, sessionId, messages, // AI SDK ModelMessage[]
steps, // AI SDK previous steps
sessionRepository, messageRepository, providerInstance, modelName, providerConfig, observer, stepIdMap) {
    try {
        ...
        const triggerResults = await checkAllTriggers(currentSession, messageRepository, sessionRepository, contextTokens);
        ...
        try {
            await createMessageStep(sessionRepository.getDatabase(), assistantMessageId, stepNumber, undefined, // metadata
            undefined, // todoSnapshot (deprecated)
            systemMessages.length > 0 ? systemMessages : undefined, currentSession.provider, // provider from session
 ...
 */
export async function completeStep(stepNumber, assistantMessageId, sessionId, stepResult, // AI SDK StepResult
currentStepParts, sessionRepository, messageRepository, tokenTracker, // StreamingTokenTracker
appContext, observer, session, cwd, stepIdMap) {
    ...
    try {
        ...
        if (stepResult.toolResults && stepResult.toolResults.length > 0) {
            for (const toolResult of stepResult.toolResults) {
                const toolPart = currentStepParts.find((p) => p.type === "tool" && p.toolId === toolResult.toolCallId);
        ...
        await recalculateTokensAtCheckpoint(sessionId, stepNumber, sessionRepository, messageRepository, tokenTracker, appContext, cwd);
        return stepNumber;
    }
    catch (error) {
...
//# sourceMappingURL=step-lifecycle.service.js.map