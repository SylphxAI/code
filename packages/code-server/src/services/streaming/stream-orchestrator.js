...
import { createAbortNotificationMessage, createAssistantMessage, createUserMessage, updateMessageStatus, } from "../message-persistence.service.js";
...
import { createStreamState, orchestrateAIStream, processAIStream, } from "./ai-orchestrator.js";
import { emitError, emitFile, emitReasoningDelta, emitReasoningEnd, emitReasoningStart, emitSessionCreated, emitSystemMessageCreated, emitTextDelta, emitTextEnd, emitTextStart, emitToolCall, emitToolError, emitToolResult, } from "./event-emitter.js";
...
import { createSessionStatusManager, } from "./session-status-manager.js";
...
export function streamAIResponse(opts) {
    return observable((observer) => {
        ...
        const executionPromise = (async () => {
            try {
                const { sessionRepository, messageRepository, aiConfig, sessionId: inputSessionId, agentId: inputAgentId, provider: inputProvider, model: inputModel, userMessageContent, abortSignal, } = opts;
                // 1. Ensure session exists (create if needed)
                let isNewSession;
                try {
                    const result = await ensureSession(sessionRepository, aiConfig, inputSessionId, inputProvider, inputModel, inputAgentId);
                ...
                const messages = await buildModelMessages(updatedSession.messages, modelCapabilities, messageRepository.getFileRepository());
                ...
                assistantMessageId = await createAssistantMessage(sessionId, messageRepository, observer);
                ...
                const stepIdMap = new Map(); // Track stepNumber → stepId mapping
                // 15. Initialize token tracking
                const cwd = process.cwd();
                const { tracker: tokenTracker, baseContextTokens } = await initializeTokenTracking(sessionId, session, messageRepository, cwd, opts.appContext);
                const tokenContext = {
                ...
                while (iterationCount < MAX_ITERATIONS) {
                    ...
                    const { fullStream, response } = await orchestrateAIStream({
                        ...
                        onStepFinish: async (stepResult) => {
                            try {
                                ...
                                await completeStep(stepNumber, assistantMessageId, sessionId, stepResult, state.currentStepParts, sessionRepository, messageRepository, tokenTracker, opts.appContext, observer, session, cwd, stepIdMap);
                        ...
                        prepareStep: async ({ steps, stepNumber, messages: stepMessages }) => {
                            ...
                            return await prepareStep(actualStepNumber, assistantMessageId, sessionId, stepMessages, steps, sessionRepository, messageRepository, providerInstance, modelName, providerConfig, observer, stepIdMap);
                        },
                    });
                    // 18. Start title generation immediately on first iteration (parallel)
                    if (needsTitle && iterationCount === 1) {
                        generateSessionTitle(opts.appContext, sessionRepository, aiConfig, updatedSession, userMessageText).catch((error) => {
                    ...
                    const stepResult = await processAIStream(fullStream, observer, state, tokenContext, callbacks, persistence);
                    ...
                    if (finalFinishReason === "stop") {
                        ...
                        if (queuedMessages.length > 0) {
                            ...
                            if (combinedContent.trim()) {
                                ...
                                const result = await createUserMessage(sessionId, [{ type: "text", content: combinedContent }], messageRepository, observer);
                                ...
                                await updateMessageStatus(assistantMessageId, previousStatus, finalFinishReason, finalUsage, messageRepository, observer);
                ...
                if (!finalUsage && !state.aborted && !hasError) {
                    ...
                    if (errorPart && errorPart.type === "error") {
                        ...
                        emitError(observer, "API request failed to generate a response. Please check your API credentials and configuration.");
                    }
                }
                // 22. Update message status
                const finalStatus = state.aborted ? "abort" : finalUsage ? "completed" : "error";
                await updateMessageStatus(assistantMessageId, finalStatus, finalFinishReason, finalUsage, messageRepository, observer);
                ...
                console.error("[StreamOrchestrator] Error message:", error instanceof Error ? error.message : String(error));
                console.error("[StreamOrchestrator] Error stack:", error instanceof Error ? error.stack : "N/A");
                ...
                if (isAbortError) {
                    ...
                    if (assistantMessageId) {
                        await updateMessageStatus(assistantMessageId, "abort", undefined, // no finishReason
                        undefined, // no usage
                        opts.messageRepository, observer);
                    }
...
//# sourceMappingURL=stream-orchestrator.js.map