 ...
 */
import { buildModelMessages, calculateBaseContextTokens, calculateModelMessagesTokens, getModel, StreamingTokenTracker, TokenCalculator, } from "@sylphx/code-core";
/**
 * Initialize token tracking for a session
 * Calculates baseline (base context + existing messages)
 */
export async function initializeTokenTracking(sessionId, session, messageRepository, cwd, appContext) {
    const calculator = new TokenCalculator(session.model);
    // Calculate base context tokens (dynamic, no cache)
    const baseContextTokens = await calculateBaseContextTokens(session.model, session.agentId, session.enabledRuleIds, cwd);
 ...
 */
export async function updateTokensFromDelta(tokenTracker, deltaText, sessionId, baseContextTokens, appContext) {
    try {
 ...
 */
export async function recalculateTokensAtCheckpoint(sessionId, _stepNumber, sessionRepository, messageRepository, tokenTracker, appContext, cwd) {
    try {
        ...
        const recalculatedBaseContext = await calculateBaseContextTokens(updatedSession.model, updatedSession.agentId, updatedSession.enabledRuleIds, cwd);
        ...
        if (updatedSession.messages && updatedSession.messages.length > 0) {
            ...
            const modelCapabilities = modelEntity?.capabilities;
            const fileRepo = messageRepository.getFileRepository();
            const modelMessages = await buildModelMessages(updatedSession.messages, modelCapabilities, fileRepo);
            recalculatedMessages = await calculateModelMessagesTokens(modelMessages, updatedSession.model);
        }
 ...
 */
export async function calculateFinalTokens(sessionId, sessionRepository, messageRepository, appContext, cwd) {
    try {
        ...
        const finalBaseContext = await calculateBaseContextTokens(finalSession.model, finalSession.agentId, finalSession.enabledRuleIds, cwd);
        // Recalculate messages tokens using current model's tokenizer
        let finalMessages = 0;
        if (finalSession.messages && finalSession.messages.length > 0) {
            ...
            const modelCapabilities = modelEntity?.capabilities;
            const fileRepo = messageRepository.getFileRepository();
            const modelMessages = await buildModelMessages(finalSession.messages, modelCapabilities, fileRepo);
...
//# sourceMappingURL=token-tracking.service.js.map