...
export const messageRouter = router({
    ...
    updateStatus: moderateProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        await ctx.messageRepository.updateMessageStatus(input.messageId, input.status, input.finishReason);
        // Note: Message updates are published by streaming.service.ts
    }),
    /**
        ...
        if (eventSessionId) {
            ...
            if (isStreaming && input.content.length > 0) {
                ...
                const attachments = input.content
                    ...
                    .filter((a) => a !== null);
                // Enqueue message
                const queuedMessage = await ctx.sessionRepository.enqueueMessage(eventSessionId, messageContent, attachments);
    ...
    enqueueMessage: moderateProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        const queuedMessage = await ctx.sessionRepository.enqueueMessage(input.sessionId, input.content, input.attachments || []);
    ...
    updateQueuedMessage: moderateProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        const updatedMessage = await ctx.sessionRepository.updateQueuedMessage(input.sessionId, input.messageId, input.content, input.attachments || []);
