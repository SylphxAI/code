					...
					case "reasoning-start":
						...
						setStreamingParts((prev: MessagePart[]) => {
							const newParts: MessagePart[] = [
								...prev,
								{ type: "reasoning", content: "", status: "active" },
							];
					...
					case "reasoning-delta":
						setStreamingParts((prev) => {
							...
							const lastReasoningIndex = newParts
								.map((p, i) => (p.type === "reasoning" && p.status === "active" ? i : -1))
								.filter((i) => i !== -1)
								.pop();
							if (
								lastReasoningIndex !== undefined &&
								newParts[lastReasoningIndex].type === "reasoning"
							) {
					...
					case "text-delta":
						setStreamingParts((prev) => {
							...
							const lastTextIndex = newParts
								.map((p, i) => (p.type === "text" && p.status === "active" ? i : -1))
								.filter((i) => i !== -1)
								.pop();
