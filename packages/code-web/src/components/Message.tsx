...
export default function Message({ message }: MessageProps) {
	...
	return (
		<div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
          ...
          ${isUser ? "bg-blue-600 text-white" : "bg-gray-800/50 text-gray-100"}
          ${isStreaming ? "animate-pulse" : ""}
        `}
			>
				{/* Role badge */}
				<div className="text-xs font-medium mb-2 opacity-70">{isUser ? "You" : "Assistant"}</div>
					...
					<div key={index} className="mb-2 last:mb-0">
						{part.type === "reasoning" && (
							<div className="mb-3 p-3 bg-black/20 rounded-lg">
								...
								<div className="text-sm opacity-80 whitespace-pre-wrap italic">{part.content}</div>
							</div>
						)}
						{part.type === "text" && (
						...
						{part.type === "tool" && (
							<div className="p-3 bg-black/20 rounded-lg text-sm mb-2">
								...
								{part.status === "active" && <div className="opacity-60">Running...</div>}
