...
export function useEventStream(options: UseEventStreamOptions = {}) {
	...
	// Ref to track subscription (lens-client .subscribe() returns an unsubscribe function)
	...
	useEffect(() => {
		...
		// lens-client .subscribe() returns an unsubscribe function directly
		...
		// Get client and subscribe using lens-client pattern
		// IMPORTANT: lens-client .subscribe() takes a SINGLE callback function, NOT an RxJS observer object
		// Pattern: client.xxx({ input: {...} }).subscribe((data) => {...}) → returns unsubscribe function
		const client = getClient();
		const unsubscribe = (client.subscribeToSession as any)({
			input: { sessionId: currentSessionId, replayLast },
		}).subscribe((storedEvent: any) => {
