...
export const contextCommand: Command = {
	...
	execute: async (commandContext) => {
		try {
			...
			// TODO: getContextInfo endpoint doesn't exist in lens queries
			// Need to either:
			// 1. Create the endpoint in packages/code-server/src/lens/queries.ts
			// 2. Use an alternative approach to calculate context info
			//
			// For now, show error message
			commandContext.setInputComponent(
				<ContextDisplay
					output="❌ Context info endpoint not yet implemented in Lens. This command is temporarily disabled."
					onComplete={() => commandContext.setInputComponent(null)}
				/>,
				"Context",
			);
